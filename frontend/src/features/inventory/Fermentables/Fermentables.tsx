...
const Fermentables: React.FC<FermentablesProps> = ({ onSelectItem }) => {
  const [searchTerm, setSearchTerm] = useState('');
  const [data, setData] = useState<Fermentable[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const result = await inventoryApi.getFermentables(searchTerm, page);
        setData(prev => page === 0 ? result.content : [...prev, ...result.content]);
        setHasMore(!result.last);
      } catch (error) {
        console.error('Failed to fetch fermentables:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="inventory-category">
      <div className="search-bar">
        <input
          type="text"
          placeholder={t('common.search_placeholder', { category: t('inventory.categories.fermentables').toLowerCase() })}
          value={searchTerm}
          ...
          return (
            ...
            >
              <div className="item-info">
                <span className="item-name">{item.type ? getFermentableIcon(item.type) : '❓'} {item.name}</span>
                <span className="item-details">{item.colorEbc} EBC • {item.type ? t(`inventory.details.types.${item.type}`) : t('common.other')}</span>
              </div>
